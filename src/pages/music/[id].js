...
export default function Music() {
    ...
    useEffect(() => {
        if (content && content.length > 0) {
          // Suponiendo que content[0].idObjeto es el id que estás buscando
          const idToFind = content[0].idObjeto;
      
          // Buscar todos los elementos que tienen la clase 'highlight'
          const highlightedElements = document.querySelectorAll('.highlight');
          
          // Eliminar la clase 'highlight' de cada elemento encontrado
          highlightedElements.forEach(element => {
            element.classList.remove('highlight');
          });
      
          // Añadir la clase 'highlight' al nuevo elemento
          const newElement = document.getElementById(idToFind);
          if (newElement) {
            newElement.classList.add('highlight');
            // Desplazar el nuevo elemento a la parte superior de la pantalla
    ...
    if (content && content.length > 0) {
        return (
          <div className='backgroundColor1' style={{ height: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
            ...
            <div className='spaceTopOnlyPhone' style={{ flex: 1, overflowY: 'auto', margin: '2%', position: 'relative', zIndex: 2 }}>
